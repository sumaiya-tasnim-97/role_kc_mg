...
def main():
    ...
    # === Plot (start from bound point only, END at end of FIRST plateau) ===
    plt.figure(figsize=(8, 6))

    # Define slice from bound point to the END of the first force plateau (inclusive)
    end_idx_plot = force_plat_end_idx + 1  # +1 because slice end is exclusive
    sl = slice(bound_idx, end_idx_plot)

    # Slice everything consistently
    d_plot            = distances[sl]
    pmf_raw_plot      = pmf_raw_desc[sl]
    pmf_smooth_plot   = pmf_smooth_desc[sl]
    pmf_w_plot        = pmf_w_smooth_desc[sl]
    pmf_sem_plot      = pmf_sem_desc[sl]
    smooth_work_plot  = smooth_work_desc[sl, :]
    closest_traj_plot = all_work_desc[sl, closest_idx]
    ...
    # Shade the FIRST force-based plateau (it will end at the right edge of the plot)
    plt.axvspan(force_plat_lo, force_plat_hi, alpha=0.14, label='First force-based plateau')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("pmf_plot_pretty.png", dpi=400, bbox_inches="tight")
    plt.show()
